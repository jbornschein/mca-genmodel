        ...
        if iteration == None:
            ...
            for indW in xrange(num_of_W):
                ...
                for ind_plot in xrange(H):
                    spl = pl.subplot(rows, cols, ind_plot+1)
                    spl.cla()
                    spl.imshow(W_tmp[ind_plot].reshape(D2,D2), interpolation='nearest', cmap=pyplot.cm.jet)
                    pyplot.axis('off')
                pl.savefig(destination + out_dir + '/W_' + iter_to_str(indW+1, num_of_W) + '.png')
        else:
            W = W[iteration,:,:]
            D2 = np.sqrt(D)
            rows = int(np.ceil(sqrt(H)))
            ...
            for ind_plot in xrange(H):
                ...
                spl.imshow(W[ind_plot].reshape(D2,D2), interpolation='nearest', cmap=pyplot.cm.jet)
                pyplot.axis('off')
                pl.savefig(destination + 'W_' + out_dir + '_iter_' + np.str(iteration+1) + '.png')
